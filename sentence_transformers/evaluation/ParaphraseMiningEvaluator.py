...
class ParaphraseMiningEvaluator(SentenceEvaluator):
    ...
    def __init__(self, sentences_map: Dict[str, str], duplicates_list: List[Tuple[str, str]] = None,
                 duplicates_dict: Dict[str, Dict[str, bool]] = None, add_transitive_closure: bool = False,
                 query_chunk_size: int = 5000, corpus_chunk_size: int = 100000, max_pairs: int = 500000,
                 top_k: int = 100, show_progress_bar: bool = False, batch_size: int = 16, name: str = '',
                 write_csv: bool = True):
        """
        ...
        for key1 in self.duplicates:
            for key2 in self.duplicates[key1]:
                if key1 in sentences_map and key2 in sentences_map and (
                        self.duplicates[key1][key2] or self.duplicates[key2][key1]):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1,
        ...
        if wandb_available and wandb.run is not None:
            wandb.log(
                dict(zip(self.csv_headers[2:], [best_precision, best_recall, best_f1, threshold, average_precision])),
                step=global_step)
